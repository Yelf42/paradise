    ...
    protected void randomTick(BlockState state, ServerLevel level, BlockPos pos, RandomSource random) {
        if (level.dimensionTypeRegistration().is(
            ...
            if (neighborsAir(level, pos) && neighborsDigitalGrass(level, pos)) {
                ...
                level.setBlock(pos, barrier ? ModBlocks.DIGITAL_GRASS_BARRIER.defaultBlockState() : ModBlocks.DIGITAL_GRASS_BLOCK.defaultBlockState(), 3);
            }
        }
    }
        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                for (int k = -1; k <= 1; k++) {
                    ...
                    if (toCheck.is(ModBlocks.DIGITAL_GRASS_BLOCK) || toCheck.is(ModBlocks.DIGITAL_GRASS_BARRIER)) return true;
                }
            }
        }
        return false;
